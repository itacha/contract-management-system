...
import { ContractDetial } from 'app/pages/contract/list/interface';

@Component({
  selector: 'usbkey-gen-list-detail',
  templateUrl: './usbkey-gen-list.detail.component.html',
})
export class USBKeyGenListDetailComponent extends BaseOfSimpleForm {
  @Input()
  item: any;
  @Input()
  ...
  ngOnInit() {
    ...
    if (this.item.applyStatus == '3') {
      this.hiddenRefuse = false;
    } else {
      this.hiddenRefuse = true;
    }
  ...
  loadMore(): void {
    let newObj = JSON.parse(this.item.contractProduct.json);
    let newStr = '';
    for (const key in newObj) {
      newStr = newObj[key] + '、' + newStr;
    }
    this.item.contractProduct.json = newStr.slice(0, newStr.length - 1);
    ...
    this.listService
      ...
      .subscribe(res => {
        ...
        res.forEach((file: any) => {
          ...
          } else if(file.productId==this.item.contractProduct.productId){
            this.profileList1s[file.productId].push(file);
            // this.profileList.push(file)
            this.profileList.push(file)
            console.log(this.profileList1s[this.item.contractProduct.productId])
          }
        });
      });
  }
