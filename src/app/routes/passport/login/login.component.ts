...
export class UserLoginComponent implements AfterViewInit, OnDestroy {
  ...
  loading = false;
  // 是否禁止使用手机登录
  distelphoneLogin = true;

  constructor(
    fb: FormBuilder,
    public passport: AppAuthService,
  ...
  ngAfterViewInit(): void {
    this.settingsService.app.name
      ? (this.titleService.suffix = this.settingsService.app.name)
      : (this.titleService.suffix = `(≧∀≦)ゞ`);
    const title = `登录`;
  ...
  submit() {
    ...
    if (this.type === 0) {
      ...
      this.passport.login(body).subscribe((response: ApiResToComponent) => {
        this.loading = false;
  ...
  open(type: string, openType: SocialOpenType = 'href') {
    ...
    else callback = 'http://192.168.4.189:4200/callback/' + type;
    switch (type) {
      case 'auth0':
